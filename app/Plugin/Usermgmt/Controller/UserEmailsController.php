...
class UserEmailsController extends AppController {
	...
	public function index() {
		...
		$this->paginate = array('limit' => 10, 'order'=>'UserEmail.id desc', 'recursive'=>0, 'conditions'=>$cond, 'fields'=>array('UserEmail.*', 'User.first_name', 'User.last_name'));
	...
	public function send($confirm=null) {
		...
		if ($this->request->isPost()) {
				...
				if ($sendValidate) {
					...
					if(is_null($confirm)) {
						if($this->request->data['UserEmail']['type']=='GROUPS') {
							...
							$cond['OR'] = $groupCond;
							$usersCount = $this->User->find('count', array('conditions'=>$cond, 'fields'=>array('User.id', 'User.first_name', 'User.last_name'), 'order'=>array('User.first_name'), 'recursive'=>-1));

							$users = $this->User->find('all', array('conditions'=>$cond, 'fields'=>array('User.id', 'User.first_name', 'User.last_name', 'User.email'), 'order'=>array('User.first_name'), 'recursive'=>-1));
						} else if($this->request->data['UserEmail']['type']=='USERS') {
							$this->request->data['UserEmail']['user_group_id']=null;
							$users = $this->User->find('all', array('conditions'=>array('User.id'=>$this->request->data['UserEmail']['user_id']), 'fields'=>array('User.id', 'User.first_name', 'User.last_name', 'User.email'), 'order'=>array('User.first_name'), 'recursive'=>-1));
					...
					if(!empty($users)) {
						if(is_null($confirm)) {
							$_SESSION['send_email_data'] = $this->request->data;
							...
							$data = $_SESSION['send_email_data'];
							$sent = $this->sendAndSaveUserEmail($data, $users);
							if($sent) {
								unset($_SESSION['send_email_data']);
			...
			if(isset($_SESSION['send_email_data'])) {
				$this->request->data = $_SESSION['send_email_data'];
				unset($_SESSION['send_email_data']);
			}
		}
	...
	public function sendToUser($userId=null, $confirm=null) {
		...
		if ($this->request->isPost()) {
			...
			if($this->RequestHandler->isAjax()) {
				...
				if ($sendValidate) {
					if(is_null($confirm)) {
						$_SESSION['send_user_email_data'] = $this->request->data;
						$this->set('data', $this->request->data);
						$template=$this->UserEmailTemplate->getTemplateById($this->request->data['UserEmail']['template']);
						$signature=$this->UserEmailSignature->getSignatureById($this->request->data['UserEmail']['signature']);
						...
						$data = $_SESSION['send_user_email_data'];
						...
						$sent = $this->sendAndSaveUserEmail($data, $users);
						if($sent) {
							unset($_SESSION['send_user_email_data']);
							if($sent) {
			...
			$this->request->data['UserEmail']['from_name'] = EMAIL_FROM_NAME;
			$this->request->data['UserEmail']['from_email'] = EMAIL_FROM_ADDRESS;
			if(isset($_SESSION['send_user_email_data'])) {
				$this->request->data = $_SESSION['send_user_email_data'];
				unset($_SESSION['send_user_email_data']);
			}
		}
		$templates = $this->UserEmailTemplate->getTemplates();
		...
		if(!empty($contactDetail)) {
			...
			if ($this->request->isPost()) {
				...
				if($this->RequestHandler->isAjax()) {
					...
					if ($sendValidate) {
						if(is_null($confirm)) {
							$_SESSION['send_reply_email_data'] = $this->request->data;
							$this->set('data', $this->request->data);
							$template=$this->UserEmailTemplate->getTemplateById($this->request->data['UserEmail']['template']);
							$signature=$this->UserEmailSignature->getSignatureById($this->request->data['UserEmail']['signature']);
							$this->set(compact('template', 'signature'));
							...
							$data = $_SESSION['send_reply_email_data'];
							$template=$this->UserEmailTemplate->getTemplateById($data['UserEmail']['template']);
							$signature=$this->UserEmailSignature->getSignatureById($data['UserEmail']['signature']);
							$fromConfig = $data['UserEmail']['from_email'];
							$fromNameConfig = $data['UserEmail']['from_name'];
	...
	On '.date('d M Y', strtotime($contactDetail['UserContact']['created'])).' at '.date('h:i A', strtotime($contactDetail['UserContact']['created'])).'<br/>'.h($contactDetail['UserContact']['name']).' wrote:</p>'.$contactDetail['UserContact']['requirement'];
				...
				if(isset($_SESSION['send_reply_email_data'])) {
					$this->request->data = $_SESSION['send_reply_email_data'];
					unset($_SESSION['send_reply_email_data']);
				}
			}
		} else {
	...
	public function view($userEmailId = null) {
		...
		if (!empty($userEmailId)) {
			...
			$userEmail = $this->UserEmail->find('first', array('conditions'=>$cond, 'recursive'=>0, 'fields'=>array('UserEmail.*', 'User.first_name', 'User.last_name')));
